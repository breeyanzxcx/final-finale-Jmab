...
<body>
  ...
  <script>
    ...
    document.getElementById('signup-form').addEventListener('submit', function(event) {
      ...
      .then(data => {
        ...
        if (data.success) {
          ...
          document.getElementById('verify-form').dataset.email = email; // Store email for verification
        } else if (data.errors && data.errors[0] === "Email is already registered or pending verification.") {
          if (confirm("This email is already registered or pending verification. Would you like us to resend the verification code?")) {
            fetch('http://localhost/jmab/final-jmab/api/users/resend', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
            ...
            .then(resendData => {
              if (resendData.success) {
                ...
                alert('Error: ' + (resendData.errors || 'Could not resend verification code.'));
              }
            });
          }
        } else {
          alert('Registration failed: ' + (data.errors || 'Please try again.'));
        }
      })
      .catch(error => {
    ...
    document.getElementById('verify-form').addEventListener('submit', function(event) {
      ...
      fetch('http://localhost/jmab/final-jmab/api/users/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: email,
      ...
      .then(data => {
        ...
        if (data.success) {
          alert('Email verified successfully! Redirecting to sign-in page.');
          window.location.href = '../HTML/sign-in.php';
        } else {
          alert('Verification failed: ' + (data.errors || 'Invalid or expired code.'));
        }
      })
      .catch(error => {
        console.error('Error:', error);
    ...
    document.getElementById('resend-code').addEventListener('click', function(event) {
      ...
      .then(data => {
        if (data.success) {
          ...
          alert('Error: ' + (data.errors || 'Could not resend verification code.'));
        }
      })
      .catch(error => {
        console.error('Error:', error);
